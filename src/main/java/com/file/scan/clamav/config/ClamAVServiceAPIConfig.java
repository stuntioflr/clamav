...
public class ClamAVServiceAPIConfig {
    ...
    public void init() {
        ...
        processBuilder.command("bash", "-c", "clambc --version");
        try {
            ...
            while ((line = reader.readLine()) != null) {
                LOGGER.info("Clambc version information = {} ",line);
            }
            int exitCode = process.waitFor();
            LOGGER.info("\nExited with error code = {}  ", exitCode);
        } catch (Exception e) {
            LOGGER.error("Exception occurred while getting clambc version = {} ", e.getMessage());
        }
    }
